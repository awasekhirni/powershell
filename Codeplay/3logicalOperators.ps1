...
forEach($num in $nums){
    if($num -eq 4){
    Write-Host $num is four.
    }
    else {
    Write-Host $num is not four.
    }
}
...
{
  Clear-Host
  "ElseIf -eq 2 branch Two"
}
else
{
  Clear-Host
  "else branch Something Else"
}
...
switch  ($var)
{
  3 {"first kid: Syed Simra Fathima"}
  "3" {"Second kid:Syed Nayyira Fathima"}
  default {"default"}
}
...
{Clear-Host; "SycliQ : systems for commuting life cycle intelligence."}

# Exceucting only shows the contents of the block, doesn't execute it 
# To actually run it, use an ampersand & in front
&{Clear-Host; "SycliQ : systems for crop life cycle intelligence."}

# You can store script blocks inside a variable
$cool = {Clear-Host; "SycliQ : systems for care life cycle intelligence."}
...
Clear-Host
$cool = {"SycliQ : systems for corporate life cycle intelligence."; " SycliQ : systems for code life cycle intelligence."}
for ($i=0;$i -lt 3; $i++)
{ 
  &$cool;
}
